...
void BSP_SensorInit(void)
{
  ...
  static const uint8_t cmds[SENSOR_INIT_CMD_LEN] = {
      SENSOR_INIT_CMD_MSB,
      SENSOR_INIT_CMD_LSB
  };
...
uint8_t *BSP_GetSensorReading(void)
{
  ...
  static uint8_t sensor_data[SENS_DATA_LEN];

  static const uint8_t cmds[SENSOR_MEASURE_CMD_LEN] = {
  ...
  BSP_ReceiveData(BSP_I2CHandle, sensor_data, SENS_DATA_LEN, SENSOR_SLAVE_ADDR, DISABLE);
